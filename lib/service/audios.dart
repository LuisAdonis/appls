...
Future<List> fetchSongsList(searchQuery) async {
  ...
  for (int i = 0; i < searchedList.length; i++) {
    searchedList[i]['title'] = searchedList[i]['title'].toString().replaceAll("&amp;", "&").replaceAll("&#039;", "'").replaceAll("&quot;", "\"");

    searchedList[i]['more_info']['singers'] = searchedList[i]['more_info']['singers'].toString().replaceAll("&amp;", "&").replaceAll("&#039;", "'").replaceAll("&quot;", "\"");
  }
  return searchedList;
}
...
//   var songsListJSON = await http.get(Uri.parse(topSongsUrl), headers: {"Accept": "application/json"});
//   var songsList = json.decode(songsListJSON.body);
//   topSongsList = songsList["list"];
//   for (int i = 0; i < topSongsList.length; i++) {
//     topSongsList[i]['title'] = topSongsList[i]['title'].toString().replaceAll("&amp;", "&").replaceAll("&#039;", "'").replaceAll("&quot;", "\"");
//     topSongsList[i]["more_info"]["artistMap"]["primary_artists"][0]["name"] =
//         topSongsList[i]["more_info"]["artistMap"]["primary_artists"][0]["name"].toString().replaceAll("&amp;", "&").replaceAll("&#039;", "'").replaceAll("&quot;", "\"");
//     topSongsList[i]['image'] = topSongsList[i]['image'].toString().replaceAll("150x150", "500x500");
//   }
//   return topSongsList;
// }
...
Future fetchSongDetails(songId) async {
  ...
  if (getMain[songId]["more_info"]["has_lyrics"] == "true") {
    ...
    var lyricsRes = await http.get(Uri.parse(lyricsUrl), headers: {"Accept": "application/json"});
    ...
    lyrics = "null";
    String lyricsApiUrl = "https://musifydev.vercel.app/lyrics/" + artist + "/" + title;
    var lyricsApiRes = await http.get(Uri.parse(lyricsApiUrl), headers: {"Accept": "application/json"});
